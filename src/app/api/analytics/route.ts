...
export async function GET() {
	try {
		...
		const totalRevenue = invoices
			...
			.reduce((acc, invoice) => acc + invoice.totalUSD, 0);
		...
		const revenueByMonth = months.map((month) => {
			const total = invoices
				.filter((invoice) => invoice.status === InvoiceStatus.PAID && monthKey(invoice.createdAt) === month.key)
				.reduce((acc, invoice) => acc + invoice.totalUSD, 0);
		...
		invoices.forEach((invoice) => {
			...
			current.count += 1;
			current.totalUSD += invoice.totalUSD;
		...
		const overdueInvoices = invoices
			.filter((invoice) => {
				...
				const isUnpaid = invoice.status !== InvoiceStatus.PAID && invoice.status !== InvoiceStatus.CANCELLED;
			...
			.map((invoice) => ({
				...
				totalUSD: formatCurrency(invoice.totalUSD),
		...
		invoices.forEach((invoice) => {
			const userId = invoice.container?.shipment?.userId;
			if (!userId) return;
			...
			entry.revenue += invoice.status === InvoiceStatus.PAID ? invoice.totalUSD : 0;
