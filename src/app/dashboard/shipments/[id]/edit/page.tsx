...
export default function EditShipmentPage() {
  ...
  const handleFetchTrackingDetails = async () => {
    ...
    try {
      ...
      toast.success('Shipping details retrieved from carrier and updated in form.');
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.weight) {
      ...
      if (isNaN(weight) || weight <= 0 || weight > 50000) {
        toast.warning('Weight must be between 0 and 50,000 lbs');
    ...
    if (formData.origin && formData.origin.trim().length > 0 && formData.origin.trim().length < 3) {
      toast.warning('Origin must be at least 3 characters');
    ...
    if (formData.destination && formData.destination.trim().length > 0 && formData.destination.trim().length < 3) {
      toast.warning('Destination must be at least 3 characters');
